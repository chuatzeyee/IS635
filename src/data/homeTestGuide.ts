        ...
        steps: [
          ...
          },
          {
            ...
            instructions: [
              ...
              '**TopAgentCommission** = `Trunc(GetTopAgent.List[0].TopAgent.TotalSales * 0.018)`  ← wrap in Trunc(): the grader TRUNCATES commission to whole dollars. Without Trunc, OutSystems rounds and you fail months where the .xxx crosses .5 (verified: a real run failed only "month 7" until Trunc was added).',
              ...
              '**TotalAgencyCommission** = `Trunc(GetTotalAgencySales.List[0].TotalAgencySales.TotalAgencySales * 0.002)`  ← also Trunc() for the same truncate-to-whole-dollars reason.',
        ...
        steps: [
          ...
          },
          {
            ...
            instructions: [
              ...
              'Chk 7 FAIL (agent commission), esp. "Wrong … for month [N]" on just one month → rounding mismatch: wrap in **Trunc(... * 0.018)**. The grader truncates to whole dollars; OutSystems rounds by default, failing the month whose fraction crosses .5. (Verified fix.) Also check TotalSales mapping (the AS TotalSales alias gotcha).',

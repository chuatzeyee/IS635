        ...
        steps: [
          ...
          },
          {
            ...
            instructions: [
              ...
              'NOTE: each field below DRILLS INTO an attribute (e.g. .AgentName). Do NOT assign the whole record — the entity field is Text/Integer/Decimal, the SQL row is a structure. The last "." segment is what makes the types match.',
              '**Year** = `DistinctMonths.List.Current.Year`',
              '**Month** = `DistinctMonths.List.Current.Month`',
              '**TopAgent** = `GetTopAgent.List[0].AgentName`  (Text ← Text)',
              '**TopAgentHomesSold** = `GetTopAgent.List[0].HomesSold`',
              '**TopAgentCommission** = `GetTopAgent.List[0].TotalSales * 0.018`',
              '**TopSellingHomeType** = `GetTopSellingHomeType.List[0].Bedrooms + " Bedroom " + GetTopSellingHomeType.List[0].PropertyType`',
              '**TotalAgencyCommission** = `GetTotalAgencySales.List[0].TotalAgencySales * 0.002`',
              'This node’s arrow loops back up to the For Each automatically (it is the last node in the loop body).',
            ],
            important:
              'If .AgentName / .HomesSold / .TotalSales do NOT appear when you dot into GetTopAgent.List[0], the Output structure TopAgent is missing those attributes — go to Data → Structures → TopAgent and add AgentName (Text), HomesSold (Integer), TotalSales (Decimal). Lists are 0-indexed → .List[0] is the single TOP-1 row (never [1]). Commission: Agent × 0.018, Agency × 0.002. Home type must render "2 Bedroom Condo" — keep the spaces in " Bedroom ".',
          },
          {

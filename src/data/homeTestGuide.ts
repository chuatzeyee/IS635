    ...
    sections: [
      ...
      },
      {
        ...
        steps: [
          ...
          {
            title: 'Step 5 — Node 4: SQL "TopAgent" (inside the loop)',
            instructions: [
              'Drag a **SQL** element onto the For Each’s **Cycle** branch.',
              'Name = **TopAgent**. Double-click → **Output** = structure **TopAgent**. Add Parameters **Year** (Integer) and **Month** (Integer). Paste Query 2 from 4.1. **Done**.',
              'Now click the TopAgent node on the canvas and set its parameter VALUES (right panel): **Year** = **DistinctMonths.List.Current.Year**, **Month** = **DistinctMonths.List.Current.Month**.',
            ],
            tip: 'The parameters were DECLARED in the editor; the VALUES are set on the node where you dropped it. Both are needed — declared so the query compiles, valued so it filters to THIS month.',
          },
          {
            title: 'Step 6 — Node 5: SQL "TopSellingHomeType" (inside the loop)',
            instructions: [
              'Drag a **SQL** element after TopAgent (still on the Cycle branch).',
              'Name = **TopSellingHomeType**. Output = structure **TopSellingHomeType**. Parameters Year, Month. Paste Query 3. **Done**.',
              'On the node, set parameter values: **Year** = **DistinctMonths.List.Current.Year**, **Month** = **DistinctMonths.List.Current.Month**.',
            ],
          },
          {
            title: 'Step 7 — Node 6: SQL "TotalAgencySales" (inside the loop)',
            instructions: [
              'Drag a **SQL** element after TopSellingHomeType.',
              'Name = **TotalAgencySales**. Output = structure **TotalAgencySales**. Parameters Year, Month. Paste Query 4. **Done**.',
              'On the node, set parameter values: **Year** = **DistinctMonths.List.Current.Year**, **Month** = **DistinctMonths.List.Current.Month**.',
            ],
          },
          {
            ...
            instructions: [
              'From the tree **Data → Entities → LocalHomeSalesSummary**, drag **CreateLocalHomeSalesSummary** onto the Cycle branch, after TotalAgencySales.',
            ...
            important:
              'Lists are 0-indexed → **.List[0]** is the single TOP-1 row (never [1]). Commission: Agent × 0.018, Agency × 0.002. Home type must render "2 Bedroom Condo" — keep the spaces in " Bedroom ". These are checkpoints 5–9.',
          },
          {
            ...
            instructions: [
              ...
              'FINAL FLOW: **Start → ClearSummary(SQL) → DistinctMonths(SQL) → For Each(DistinctMonths.List){ TopAgent → TopSellingHomeType → TotalAgencySales → CreateLocalHomeSalesSummary } → End**.',
            ],

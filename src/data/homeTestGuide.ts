    ...
    sections: [
      {
        ...
        steps: [
          {
            title: 'Build the sync flow',
            instructions: [
              '**Logic** → right-click **Server Actions** → **Add Server Action** → **SyncHomesAndSales**',
              '— Homes —',
              'Drag **Run Server Action** → pick the consumed **GetHomes** method (this is the actual API call → checkpoint 1)',
              'Drag a **For Each** over **GetHomes.Response.<list>** (the list attribute inside the response)',
              'Inside the loop drag **CreateOrUpdateLocalHome** → Source = the current API record (map each field; copy-paste matched names so this is 1-to-1)',
              '— Home Sales —',
              'Drag **Run Server Action** → pick **GetHomeSales** (→ checkpoint 2)',
              'Drag a **For Each** over **GetHomeSales.Response.<list>**',
              'Inside, drag **CreateOrUpdateLocalHomeSales** → Source = current API record',
              'Connect to **End**',
            ],
            important:
              'Use **CreateOrUpdate** (keyed by the API id) so re-running does not duplicate rows. If HomeSalesData has no natural key, add a **DeleteAll**/clear step before the loop (or a bootstrap aggregate) so you start clean each run.',
          },
          {

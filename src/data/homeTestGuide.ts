    ...
    sections: [
      {
        ...
        steps: [
          ...
          {
            title: 'Step C — Discover the exact name of the returned list',
            instructions: [
              'You need to know what GetHomes hands back so you can loop over it. Do this ONCE and write the name down.',
              'In the left tree, expand **Integrations → REST → IS467LabTest_HomeSales → GetHomes**. Under it you see **Input Parameters** and **Output Parameters**.',
              'Open **Output Parameters** — there is one output, usually called **Response**.',
              'Expand **Response**. It is a Structure. ONE of its attributes will be a LIST (shown with a stacked/table icon) — that is the list of homes.',
              'Note that attribute’s exact name. Likely candidates for this lab: **HomeData**, **HomeList**, or **List**. Whatever it is, the full path to the rows is **GetHomes.Response.<thatName>**.',
              'WORKED EXAMPLE used below: assume the list attribute is named **HomeData**, so the rows live at **GetHomes.Response.HomeData**. (Substitute your real name if it differs.)',
            ],
            tip: 'Fast alternative: skip the tree. In Step D when you set the For Each List, just type "GetHomes.Response." in the expression box — autocomplete lists every attribute and the list-typed one (table icon) is the rows.',
          },
          {
            title: 'Step D — Add a For Each to loop the homes',
            instructions: [
              'In the **Toolbox** (far left), find **For Each** (a circular-arrow / loop icon). Search "For Each" if needed.',
              'Drag **For Each** onto the line just AFTER the GetHomes node. Drop when the line highlights — it connects Start → GetHomes → ForEach → End.',
              'Click the **For Each** node to select it. On the right, **Properties** shows a **List** property (currently empty).',
              'Click the **List** value box → the expression editor opens. Type: **GetHomes.Response.HomeData** (or your real list name from Step C). Click **Close/Done**.',
              'The For Each will now run its body once per home in that list.',
            ],
            important:
              'The **List** property is the ONLY thing you must set on the For Each. If autocomplete shows a red underline, the attribute name is wrong — re-check Step C for the real list name.',
          },
          {
            title: 'Step E — Inside the loop, save each home to LocalHome',
            instructions: [
              'From the **Toolbox**, you do NOT find a "CreateOrUpdate" tool to drag. Instead: in the left tree expand **Data → Entities → LocalHome**. Under it are the auto-generated actions: **CreateLocalHome, CreateOrUpdateLocalHome, GetLocalHome, UpdateLocalHome, DeleteLocalHome**.',
              'Drag **CreateOrUpdateLocalHome** from the tree onto the loop body — i.e. drop it on the small connector that hangs DOWN from the For Each (the "Cycle" branch), not on the main line.',
              'Click the **CreateOrUpdateLocalHome** node. In **Properties** it has a **Source** input of type "LocalHome Record".',
              'Click the **Source** value → expression editor → type the current row: **GetHomes.Response.HomeData.Current** (".Current" = the home for this iteration). Click Done.',
              'Because LocalHome’s attributes were copy-pasted from HomeData (Phase 2.1), assigning the whole record maps every field automatically — you do not map fields one by one.',
              'Make sure the node’s out-arrow loops back to the For Each (it should auto-connect; if not, drag from the node back to the For Each).',
            ],
            tip: 'If .Current does not autocomplete, click the For Each first to confirm its List is set — .Current only exists once the loop has a valid List.',
          },
          {
            title: 'Step F — Repeat for Home Sales, then close the flow',
            instructions: [
              'After the Homes For Each, drag another **Run Server Action** onto the main line → pick **GetHomeSales** (→ checkpoint 2).',
              'Drag a second **For Each** after it → set its **List** = **GetHomeSales.Response.HomeSalesData** (find the real list-attribute name the same way as Step C).',
              'Into THAT loop body, drag **CreateOrUpdateLocalHomeSales** (from Data → Entities → LocalHomeSales) → set **Source** = **GetHomeSales.Response.HomeSalesData.Current**.',
              'Make sure the last node connects to **End**.',
              'FINAL FLOW: **Start → GetHomes → For Each(Homes){CreateOrUpdateLocalHome} → GetHomeSales → For Each(Sales){CreateOrUpdateLocalHomeSales} → End**',
              'Press **Ctrl+S** then **1-Click Publish** to check it compiles.',
            ],
            important:
              'Use **CreateOrUpdate** (keyed by the API id, e.g. HomeId) so re-running does not duplicate rows. If HomeSalesData has no id to upsert on, drag **DeleteAll** for LocalHomeSales onto the line BEFORE its loop so each run starts clean — otherwise checkpoint 4 (record count) drifts upward on every reload.',
          },
          {

...
export const buildPhases: readonly BuildPhase[] = [
  ...
  // ──────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      // ── 6.2: Create Structures — Request Types ──
      {
        id: '6.2',
        title: 'Create Structures — Request Types',
        summary:
          'Create the **InputContentItem**, **InputMessage**, and **ResponsesAPIRequest** Structures that define the JSON shapes sent to the OpenAI Responses API.',
        steps: [
          {
            title: 'Understand Structures and JSON mapping',
            instructions: [
              'In OutSystems, a **Structure** maps to a **JSON object**',
              'A **List** of a Structure maps to a **JSON array**',
              'The **Name in JSON** property on each attribute controls the exact key name in the serialized JSON',
              'OpenAI uses **snake_case** for its JSON keys (e.g., **max_output_tokens**, **file_url**)',
              'OutSystems attribute names use **PascalCase** (e.g., **MaxOutputTokens**, **FileUrl**)',
              'You MUST set **Name in JSON** on every attribute to match the OpenAI snake_case format — otherwise the API call will fail with a 400 error',
              'The Responses API uses **input** instead of **messages** and **max_output_tokens** instead of **max_tokens** — be careful not to use the old Chat Completions names',
            ],
            important:
              'The **Name in JSON** property is case-sensitive. If OpenAI expects **max_output_tokens** and you send **MaxOutputTokens** or **max_tokens**, the API will reject your request.',
          },
          {
            title: 'Create the InputContentItem Structure',
            instructions: [
              'Click the **Data** tab on the left panel',
              'Expand the **Structures** folder',
              'Right-click **Structures** > select **Add Structure**',
              'Set **Name** to **InputContentItem**',
              'Right-click **InputContentItem** > select **Add Structure Attribute**',
              'Set **Name** to **Type**',
              'Set **Data Type** to **Text**',
              'In the **Properties** panel on the right, find **Name in JSON**',
              'Set **Name in JSON** to **type**',
              'Set **Is Mandatory** to **Yes**',
              'Right-click **InputContentItem** again > select **Add Structure Attribute**',
              'Set **Name** to **Text**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **text**',
              'Set **Is Mandatory** to **No**',
              'Right-click **InputContentItem** again > select **Add Structure Attribute**',
              'Set **Name** to **FileUrl**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **file_url**',
              'Set **Is Mandatory** to **No**',
            ],
            tip: 'The **Type** attribute will hold either **"input_text"** (for plain text content) or **"input_file"** (for file attachments like PDFs or images). When Type is **"input_text"**, fill the **Text** attribute. When Type is **"input_file"**, fill the **FileUrl** attribute with a URL to the file.',
          },
          {
            title: 'Create the InputMessage Structure',
            instructions: [
              'Right-click **Structures** > select **Add Structure**',
              'Set **Name** to **InputMessage**',
              'Right-click **InputMessage** > select **Add Structure Attribute**',
              'Set **Name** to **Role**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **role**',
              'Set **Is Mandatory** to **Yes**',
              'Right-click **InputMessage** > select **Add Structure Attribute**',
              'Set **Name** to **Content**',
              'Set **Data Type** to **InputContentItem List**',
              'Set **Name in JSON** to **content**',
              'Set **Is Mandatory** to **Yes**',
            ],
            tip: 'The **Role** attribute will always be **"user"** for user messages. The **Content** attribute is a **List** of **InputContentItem** — this means a single message can contain multiple content items (e.g., text plus a file attachment).',
          },
          {
            title: 'Create the ResponsesAPIRequest Structure',
            instructions: [
              'Right-click **Structures** > select **Add Structure**',
              'Set **Name** to **ResponsesAPIRequest**',
              'Right-click **ResponsesAPIRequest** > select **Add Structure Attribute**',
              'Set **Name** to **Model**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **model**',
              'Set **Is Mandatory** to **Yes**',
              'Right-click **ResponsesAPIRequest** > select **Add Structure Attribute**',
              'Set **Name** to **Instructions**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **instructions**',
              'Set **Is Mandatory** to **No**',
              'Right-click **ResponsesAPIRequest** > select **Add Structure Attribute**',
              'Set **Name** to **Input**',
              'Set **Data Type** to **InputMessage List**',
              'Set **Name in JSON** to **input**',
              'Set **Is Mandatory** to **Yes**',
              'Right-click **ResponsesAPIRequest** > select **Add Structure Attribute**',
              'Set **Name** to **MaxOutputTokens**',
              'Set **Data Type** to **Integer**',
              'Set **Name in JSON** to **max_output_tokens**',
              'Set **Is Mandatory** to **No**',
              'Right-click **ResponsesAPIRequest** > select **Add Structure Attribute**',
              'Set **Name** to **Temperature**',
              'Set **Data Type** to **Decimal**',
              'Set **Name in JSON** to **temperature**',
              'Set **Is Mandatory** to **No**',
            ],
            important:
              'Common mistake: the **Name in JSON** for **MaxOutputTokens** must be **max_output_tokens** (snake_case). Do NOT use **max_tokens** (that was the old Chat Completions API name) — OpenAI will reject the request with a 400 Bad Request.',
            tip: 'The **Instructions** attribute is where the system prompt goes — this replaces the old approach of adding a message with role **"system"**. With the Responses API, the system prompt is a top-level field, not part of the input messages array. Also note: with this API, you can send files (PDFs, images) by adding content items with **type: "input_file"** and a **file_url** — a powerful capability for document-aware AI features.',
          },
        ],
      },

      // ── 6.3: Create Structures — Response Types ──
      {
        id: '6.3',
        title: 'Create Structures — Response Types',
        summary:
          'Create the **OutputContentItem**, **OutputMessage**, **ResponseUsage**, **ResponsesAPIResponse**, and **OpenAIErrorDetail** Structures that define the JSON shapes returned by the OpenAI Responses API.',
        steps: [
          {
            title: 'Create the OutputContentItem Structure',
            instructions: [
              'Right-click **Structures** > select **Add Structure**',
              'Set **Name** to **OutputContentItem**',
              'Right-click **OutputContentItem** > select **Add Structure Attribute**',
              'Set **Name** to **Type**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **type**',
              'Set **Is Mandatory** to **No**',
            ...
            tip: 'The **Type** attribute will be **"output_text"** for text responses. The **Text** attribute holds the actual AI-generated response text.',
          },
          {
            title: 'Create the OutputMessage Structure',
            instructions: [
              'Right-click **Structures** > select **Add Structure**',
              'Set **Name** to **OutputMessage**',
              'Right-click **OutputMessage** > select **Add Structure Attribute**',
              'Set **Name** to **Id**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **id**',
              'Set **Is Mandatory** to **No**',
              'Right-click **OutputMessage** > select **Add Structure Attribute**',
              'Set **Name** to **Type**',
              'Set **Data Type** to **Text**',
              ...
              'Set **Name** to **Status**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **status**',
              'Set **Is Mandatory** to **No**',
              'Right-click **OutputMessage** > select **Add Structure Attribute**',
              'Set **Name** to **Content**',
              'Set **Data Type** to **OutputContentItem List**',
              'Set **Name in JSON** to **content**',
              'Set **Is Mandatory** to **No**',
              'Right-click **OutputMessage** > select **Add Structure Attribute**',
              'Set **Name** to **Role**',
            ...
            tip: 'The **Type** attribute will be **"message"** for standard responses. The **Status** attribute will be **"completed"** when the AI finished generating. The **Content** attribute is a **List** of **OutputContentItem** — the actual response text is inside the first item of this list.',
          },
          {
            title: 'Create the ResponseUsage Structure',
            instructions: [
              'Right-click **Structures** > select **Add Structure**',
              'Set **Name** to **ResponseUsage**',
              'Right-click **ResponseUsage** > select **Add Structure Attribute**',
              'Set **Name** to **InputTokens**',
              'Set **Data Type** to **Integer**',
              'Set **Name in JSON** to **input_tokens**',
              'Set **Is Mandatory** to **No**',
              'Right-click **ResponseUsage** > select **Add Structure Attribute**',
              'Set **Name** to **OutputTokens**',
              'Set **Data Type** to **Integer**',
              'Set **Name in JSON** to **output_tokens**',
              'Set **Is Mandatory** to **No**',
              'Right-click **ResponseUsage** > select **Add Structure Attribute**',
              'Set **Name** to **TotalTokens**',
              'Set **Data Type** to **Integer**',
              'Set **Name in JSON** to **total_tokens**',
              'Set **Is Mandatory** to **No**',
            ],
            tip: 'The Responses API uses **input_tokens** and **output_tokens** instead of the old **prompt_tokens** and **completion_tokens**. The **total_tokens** field is still the sum of both.',
          },
          {
            title: 'Create the OpenAIErrorDetail Structure',
            instructions: [
              'Right-click **Structures** > select **Add Structure**',
              'Set **Name** to **OpenAIErrorDetail**',
              'Right-click **OpenAIErrorDetail** > select **Add Structure Attribute**',
              'Set **Name** to **Message**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **message**',
              'Set **Is Mandatory** to **No**',
              'Right-click **OpenAIErrorDetail** > select **Add Structure Attribute**',
              'Set **Name** to **Type**',
              'Set **Data Type** to **Text**',
              ...
              'Set **Name** to **Code**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **code**',
              'Set **Is Mandatory** to **No**',
            ],
            tip: 'This Structure captures error details that OpenAI returns when something goes wrong (e.g., invalid API key, rate limit exceeded). In the Responses API, the error is an **inline field** on the response object itself — you do NOT need a separate **OpenAIErrorResponse** wrapper Structure.',
          },
          {
            title: 'Create the ResponsesAPIResponse Structure',
            instructions: [
              'Right-click **Structures** > select **Add Structure**',
              'Set **Name** to **ResponsesAPIResponse**',
              'Right-click **ResponsesAPIResponse** > select **Add Structure Attribute**',
              'Set **Name** to **Id**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **id**',
              'Set **Is Mandatory** to **No**',
              ...
              'Set **Name** to **Object**',
              'Set **Data Type** to **Text**',
              'Set **Name in JSON** to **object**',
              'Set **Is Mandatory** to **No**',
              'Right-click **ResponsesAPIResponse** > select **Add Structure Attribute**',
              'Set **Name** to **CreatedAt**',
              'Set **Data Type** to **Long Integer**',
              'Set **Name in JSON** to **created_at**',
              'Set **Is Mandatory** to **No**',
              'Right-click **ResponsesAPIResponse** > select **Add Structure Attribute**',
              'Set **Name** to **Status**',
              'Set **Data Type** to **Text**',
              ...
              'Right-click **ResponsesAPIResponse** > select **Add Structure Attribute**',
              'Set **Name** to **Output**',
              'Set **Data Type** to **OutputMessage List**',
              'Set **Name in JSON** to **output**',
              'Set **Is Mandatory** to **No**',
              'Right-click **ResponsesAPIResponse** > select **Add Structure Attribute**',
              'Set **Name** to **Usage**',
              'Set **Data Type** to **ResponseUsage**',
              'Set **Name in JSON** to **usage**',
              'Set **Is Mandatory** to **No**',
              'Right-click **ResponsesAPIResponse** > select **Add Structure Attribute**',
              'Set **Name** to **Error**',
              'Set **Data Type** to **OpenAIErrorDetail**',
              'Set **Name in JSON** to **error**',
              'Set **Is Mandatory** to **No**',
            ],
            important:
              'Do NOT set any attribute on **ResponsesAPIResponse** to **Mandatory**. The response shape from OpenAI may vary on errors, and mandatory attributes will cause deserialization failures. Note that the **Error** attribute is an inline field on this response object — when an error occurs, OpenAI populates this field directly instead of returning a completely different JSON shape.',
          },
        ],
      },

      // ── 6.4: Create Site Properties ──
      {
        id: '6.4',
        title: 'Create Site Properties',
        summary:
      ...
      // ── 6.5: Consume the OpenAI REST API ──
      {
        id: '6.5',
        title: 'Consume the OpenAI REST API',
        summary:
          'Add the OpenAI Responses endpoint as a **Consumed REST API** and configure the **OnBeforeRequest** callback to inject the Authorization header.',
        steps: [
          {
          ...
          },
          {
            ...
            tip: 'OutSystems auto-generates Structures from both samples. You can either use the auto-generated Structures directly, or delete them and rewire the consumed method to the manual Structures you created in Sections 6.2 and 6.3.',
          },
          {
            title: 'Add the Authorization header via OnBeforeRequest',
            instructions: [
              'In the **Logic** tab, expand **Integrations** > **REST** > find the consumed REST API parent node (it may be named **OpenAI_API** or similar — this is the parent, not the individual method)',
              'Click on the consumed REST API parent node to select it',
              'In the **Properties** panel on the right, find the **On Before Request** property',
              'Click the dropdown and select **New Server Action** (OutSystems will create a callback action)',
              'The new Server Action opens automatically in the flow editor',
              'Right-click the Server Action > select **Add Local Variable**',
              'Set **Name** to **AuthHeader**',
              'Set **Data Type** to **HTTPHeader**',
              'Drag an **Assign** node after the **Start** node',
              'Add the first assignment — set **Target**: **AuthHeader.Name**',
              'Set **Value**: **"Authorization"**',
              'Add the second assignment — set **Target**: **AuthHeader.Value**',
              'Set **Value**: **"Bearer " + Site.OpenAI_APIKey**',
              'Add the third assignment — set **Target**: **Request.Headers**',
              'Set **Value**: **ListAppend(Request.Headers, AuthHeader)**',
              'Add the fourth assignment — set **Target**: **AuthHeader.Name**',
              'Set **Value**: **"Content-Type"**',
              'Add the fifth assignment — set **Target**: **AuthHeader.Value**',
              'Set **Value**: **"application/json"**',
              'Add the sixth assignment — set **Target**: **Request.Headers**',
              'Set **Value**: **ListAppend(Request.Headers, AuthHeader)**',
              'Connect the **Assign** node to the **End** node',
            ],
            important:
              'The header value must be exactly **"Bearer "** followed by the API key — note the space after **Bearer**. If you omit the space, OpenAI will return a 401 Unauthorized error.',
          },
          {
            title: 'Alternative: pass Authorization as an Input Parameter',
            instructions: [
              'Instead of using **OnBeforeRequest**, you can add **Authorization** as an **Input Parameter** directly on the consumed REST method',
              'Set **Send In** to **Header**',
              'Each time you call the method, pass **"Bearer " + Site.OpenAI_APIKey** as the value',
              'This approach is simpler but requires you to pass the header value on every call',
            ],
            tip: 'The **OnBeforeRequest** approach is preferred because it centralizes the authorization logic in one place. If you ever add more OpenAI methods, they all inherit the header automatically.',
          },
        ],
      },

      // ── 6.6: Build the ChatCompletion Server Action ──
      {
        id: '6.6',
        title: 'Build the ChatCompletion Server Action',
        summary:
          'Create a public **ChatCompletion** Server Action that accepts a user message and system prompt, calls the consumed OpenAI API, and returns the AI response with error handling.',
        steps: [
          {
          ...
          },
          {
            ...
            instructions: [
              ...
              'Set **Data Type** to **InputContentItem**',
              'Right-click **ChatCompletion** > select **Add Local Variable**',
              'Set **Name** to **InputMsg**',
              'Set **Data Type** to **InputMessage**',
              'Right-click **ChatCompletion** > select **Add Local Variable**',
              'Set **Name** to **InputList**',
              'Set **Data Type** to **InputMessage List**',
            ],
            tip: 'Notice there is no **SystemMsg** variable — in the Responses API, the system prompt is passed as a top-level **Instructions** parameter, not as a message in the input array.',
          },
          {
          ...
          {
            title: 'Step 2: Assign node — BuildInput',
            instructions: [
              'Drag a second **Assign** node after **SetDefaults**',
              'Rename this Assign node to **BuildInput**',
              'Add the first assignment:',
              'Set **Target**: **TextContent.Type**',
              'Set **Value**: **"input_text"**',
              'Add the second assignment:',
              'Set **Target**: **TextContent.Text**',
              'Set **Value**: **UserMessage**',
              'Add the third assignment:',
              'Set **Target**: **ContentList**',
              'Set **Value**: **ListAppend(ContentList, TextContent)**',
              'Add the fourth assignment:',
              'Set **Target**: **InputMsg.Role**',
              'Set **Value**: **"user"**',
              'Add the fifth assignment:',
              'Set **Target**: **InputMsg.Content**',
              'Set **Value**: **ContentList**',
              'Add the sixth assignment:',
              'Set **Target**: **InputList**',
              'Set **Value**: **ListAppend(InputList, InputMsg)**',
            ],
            important:
              'The system prompt is NOT added as a message in the input array. It will be passed as the **Instructions** parameter directly on the API call in the next step. Only the user message goes into the **InputList**.',
          },
          {
            title: 'Step 3: Call the Consumed CreateResponse API',
            instructions: [
              'Drag the **CreateResponse** consumed REST method from the **Logic** tab > **Integrations** > **REST** and drop it after the **BuildInput** Assign node',
            ...
            tip: 'The parameter names on the left side are the consumed REST method inputs. The values on the right side are your Server Action local variables and input parameters. Notice that **Instructions** maps to **SystemPrompt** — this is how the system prompt gets passed to the Responses API as a top-level field instead of as a message.',
          },
          {
            title: 'Step 4: Assign node — SetOutput',
            instructions: [
              'Drag a third **Assign** node after the **CreateResponse** call',
            ...
            important:
              'The response path is **CreateResponseResponse.Output.Current.Content.Current.Text** — this navigates through the **Output** list (first message), then into its **Content** list (first content item), and finally reads the **Text** attribute. This is deeper than the old Chat Completions path because the Responses API nests content items inside output messages.',
          },
          {
      ...
      // ── 6.7: Expose as REST API (Optional) ──
      {
        id: '6.7',
      ...
      // ── 6.8: Publish, Configure & Test ──
      {
        ...
        steps: [
          ...
          },
          {
            ...
            instructions: [
              'If you created the Exposed REST API in Section 6.7, right-click the **OpenAIService** REST API in **Service Studio** > select **Open Documentation**',
            ...
            tip: 'If you get a **401 Unauthorized** error, double-check that the API key is set correctly in Service Center and that the **OnBeforeRequest** callback includes the **"Bearer "** prefix with a space.',
          },
        ],
      },

      // ── 6.9: Consume from CareConnect UI ──
      {
        id: '6.9',
        ...
        steps: [
          {
            ...
            instructions: [
              '**400 Bad Request**: Check **Name in JSON** on every Structure attribute — they must be **snake_case** (e.g., **max_output_tokens**, not **MaxOutputTokens**). Also verify you are using the Responses API field names (**input**, **instructions**, **max_output_tokens**), not the old Chat Completions names (**messages**, **max_tokens**)',
              '**401 Unauthorized**: The API key is not set in **Service Center**, or the **Authorization** header is missing the **"Bearer "** prefix (with a space after Bearer)',
              ...
              '**Status is not "completed"**: Always check that **CreateResponseResponse.Status** is **"completed"** before reading the output. If the status is **"failed"**, the **Error** field on the response will contain details about what went wrong',
              ...
              '**"Structure mismatch" on response deserialization**: OpenAI occasionally updates its response format — update the consumed REST API response sample JSON to match the new shape',
            ...
            tip: 'The Responses API also supports file inputs. As a bonus feature, you can send PDFs or images to the AI by adding content items with **type: "input_file"** and a **file_url** in the **InputContentItem** Structure. This is useful for document-aware features like care plan analysis.',
          },
          {
            ...
            instructions: [
              ...
              'Verify: You created **8 Structures** with correct **Name in JSON** values (all snake_case): **InputContentItem**, **InputMessage**, **ResponsesAPIRequest**, **OutputContentItem**, **OutputMessage**, **ResponseUsage**, **ResponsesAPIResponse**, **OpenAIErrorDetail**',
              ...
              'Verify: The **OnBeforeRequest** callback injects the **Authorization: Bearer {APIKey}** header',
              'Verify: The **ChatCompletion** Server Action has **Public** set to **Yes**',
              'Verify: The response text is read from **CreateResponseResponse.Output.Current.Content.Current.Text** (not the old **.Choices.Current.Message.Content** path)',

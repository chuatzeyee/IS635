...
import type { PlaybookSection, PlaybookStep } from '../data/examPlaybook'
...
function StepCard({ step, stepIndex }: { readonly step: PlaybookStep; readonly stepIndex: number }) {
  return (
...
function SectionCard({ section, index, defaultOpen }: { readonly section: PlaybookSection; readonly index: number; readonly defaultOpen: boolean }) {
...
export default function Playbook() {
  ...
  const phase = playbookPhases.find((p) => p.id === activePhase)!

  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      <h1 className="text-3xl font-bold text-ink mb-2 tracking-tight">Exam Playbook</h1>
      <p className="text-ink-muted mb-6">
        Speed reference for the open-book practical exam (Sessions 1&ndash;6). Start with Phase 1
        (First 30 Min), then jump to the layer you&apos;re building. Generic patterns &mdash; map
        them onto whatever scenario the exam gives you.
      </p>
      ...
      <div className="space-y-3">
        {phase.sections.map((section, idx) => (
          <SectionCard key={section.id} section={section} index={idx} defaultOpen={activePhase === 1} />
        ))}
      </div>
    </div>
  )

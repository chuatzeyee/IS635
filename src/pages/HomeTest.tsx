...
import type { HTSection, HTStep } from '../data/homeTestGuide'
...
function renderInline(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) {
      return (
        <code key={i} className="font-mono text-[12px] bg-base/70 text-glow border border-edge rounded px-1 py-0.5 break-all">
          {part.slice(1, -1)}
        </code>
      )
    }
...
function StepCard({ step, stepIndex }: { readonly step: HTStep; readonly stepIndex: number }) {
  return (
    <div className="pl-4 border-l-2 border-edge">
      ...
      {step.tip && (
        <div className="mt-2 ml-7 flex items-start gap-2 px-3 py-2 rounded-lg bg-glow-dim/30 border border-glow/15">
          ...
          <p className="text-xs text-glow/90 leading-relaxed">{renderInline(step.tip)}</p>
        </div>
      )}
      {step.important && (
        <div className="mt-2 ml-7 flex items-start gap-2 px-3 py-2 rounded-lg bg-[#422006]/40 border border-[#f59e0b]/15">
          ...
          <p className="text-xs text-[#fbbf24]/90 leading-relaxed">{renderInline(step.important)}</p>
        </div>
      )}
    </div>
  )
...
function SectionCard({ section, index, defaultOpen }: { readonly section: HTSection; readonly index: number; readonly defaultOpen: boolean }) {
...
export default function HomeTest() {
  ...
  const phase = homeTestPhases.find((p) => p.id === activePhase)!

  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      <h1 className="text-3xl font-bold text-ink mb-2 tracking-tight">Home Test Lab</h1>
      <p className="text-ink-muted mb-6">
        Comprehensive build guide for the trial Midterm Lab (Real Estate Home Sales). Consume the
        REST API, compute the monthly sales summary, submit, and pass all 9 checkpoints. Work
        Phase 0 → 6 in order.
      </p>

      <div className="flex gap-2 mb-6 flex-wrap">
        {homeTestPhases.map((p) => (
          <button
            key={p.id}
      ...
      <div className="text-sm text-ink-secondary mb-6 leading-relaxed">
        {phase.description.split(/(\*\*[^*]+\*\*)/).map((part, i) =>
      ...
      <div className="space-y-3">
        {phase.sections.map((section, idx) => (
          <SectionCard key={section.id} section={section} index={idx} defaultOpen={activePhase === 0} />
        ))}
      </div>
    </div>
  )

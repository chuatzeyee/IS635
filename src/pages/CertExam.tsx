...
const domainFilters: readonly { readonly label: string; readonly value: DomainFilter }[] = [
  { label: 'All', value: 'all' },
...
export default function CertExam() {
  ...
  const filtered = useMemo(() => {
    const base = filter === 'all' ? certQuestions : certQuestions.filter((q) => q.domain === filter)
    if (order.length > 0) {
  ...
  const streak = useMemo(() => {
    ...
    for (const q of filtered) {
      const ans = answers.get(q.id)
      if (ans?.revealed) {
        if (ans.selectedIndex === q.correctIndex) current++
        else current = 0
      }
    }
    return current
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      <div className="flex items-center justify-between mb-2">
        <h1 className="text-3xl font-bold text-ink tracking-tight">Cert Exam</h1>
      ...
      <p className="text-ink-muted mb-6">
        OutSystems Associate Reactive Developer — certification-style practice.
        Product-mechanics questions across the official exam domains.
      </p>

      <div className="flex items-center gap-2 mb-6 flex-wrap">
        {domainFilters.map(({ label, value }) => (
          <button
            key={label}
            onClick={() => {
              setFilter(value)
      ...
      <div className="bg-surface border border-edge rounded-lg px-5 py-4 mb-6">
        <ProgressBar correct={score.correct} attempted={score.attempted} total={filtered.length} />
        {streak >= 3 && (
          ...
          return (
            ...
            >
              ...
              {revealed && selected === q.correctIndex && (
                <p className="text-sm text-correct font-medium animate-scale-in">Correct!</p>
              )}
              {revealed && selected !== q.correctIndex && (
